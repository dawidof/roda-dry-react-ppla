...
namespace :db do
  ...
  task version: :setup do
    version =
      if connection.tables.include?(:schema_migrations)
        # puts connection[:schema_migrations].order(:filename).last
  ...
  desc "Generate a new migration file `db:generate[create_users]`"
  task :generate, [:name] do |_, args|
    ...
    filename = File.join(migrations_dir, "#{timestamp}_#{name}.rb")
  ...
  def migrate(version = nil)
    ...
    Sequel::Migrator.apply(connection, migrations_dir, version)
